...
let config_path = process.env.APPDATA || (process.platform == 'darwin' ? process.env.HOME + '/Library/Application Support/zendesk-cli' : '/var/local/zendesk-cli'); 
let config = null;
let client = null;

try {
    config = fs.readJsonSync(`${config_path}/config.json`);
    ...
    console.log();
    console.log(`    No config file found. Try \`zd init\``);
    console.log();
...
function createMacro (data, options) {
    client.macros.create(data, (err, req, result) => {
        ...
        data.macro.actions.push({
            field: `current_tags`,
            value: `macro_${result.macro.id}`
        });

        client.macros.update(result.macro.id, data, (err, req, result) => {
            if (err) throw err;
